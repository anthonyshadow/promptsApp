...
type FrontierPoint = EvalRunDetailResponse["frontier_points"][number];

function createLocalEvalDetail(status: EvalRun["status"] = demoEvalRun.status): EvalRunDetailResponse {
  const evalRun = {
    ...demoEvalRun,
    status
  };
...
function createLocalFrontierPoints(evalRun: EvalRun, results: EvalResult[]): FrontierPoint[] {
  const passingNonBaseline = results.filter((result) => result.verdict === "pass" && !isBaselineResult(evalRun, result));
...
function compareEvalResults(left: EvalResult, right: EvalResult): number {
...
function shouldPollStatus(status: EvalRun["status"]): boolean {
  return status === "queued" || status === "running" || status === "retrying" || status === "rate_limited";
}

function getStatusTone(status: EvalRun["status"]): "good" | "warn" {
  if (status === "complete") {
    return "good";
  }
...
function formatEvalStatus(status: EvalRun["status"]): string {
  switch (status) {
    case "queued":
      return "Queued";
    case "running":
...
function formatFrontierRole(role: FrontierPoint["role"]): string {
  switch (role) {
    case "baseline":
      return "Baseline";
    case "safe":
...
function getCostProxy(result: EvalResult): number {
