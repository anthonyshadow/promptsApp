...
function createManualTestCaseRequest(input: {
  name: string;
  ...
  pattern: string;
  mustPass: boolean;
}): Parameters<PromptOptsApiClient["createTestCase"]>[1] {
...
function createCsvTestCaseRequest(
  draft: { name: string; inputVariables: Record<string, unknown>; expectedOutput: unknown },
  contract: QualityContract,
  index: number
): Parameters<PromptOptsApiClient["createTestCase"]>[1] {
  const baseCheck =
...
function createFallbackQualityCheck(index: number): QualityCheckDefinition {
  return {
...
function createManualCheckDescription(
  checkType: QualityCheckDefinition["type"],
  expectedValue: string
): string {
  switch (checkType) {
...
function parseJsonObjectText(value: string): Record<string, unknown> {
...
function formatTestCaseStatus(testCase: TestCase): string {
...
function formatExpectedOutput(value: unknown): string {
