...
| Real sudo lifecycle | Trust And Security E | complete | Sudo start/status/end routes persist action-scoped, reason-coded, time-boxed grants; start requires MFA recheck, expired/wrong-action grants reject dangerous actions, UI shows sudo modal/banner state, and lifecycle events are audited. | Keep break-glass and raw reveal payload flows placeholder-only until encrypted payload/provider-key work lands. | Dangerous actions only pass with valid unexpired sudo and reason code. |
