...
| Real sudo lifecycle | Trust And Security E | in_progress | Dangerous routes read durable approved `sudo_requests`; mock sudo headers are rejected. | Add sudo request creation, reason binding, approval/denial/revocation, expiry transitions, UI, and audit events. | Dangerous actions only pass with valid unexpired sudo and reason code. |
